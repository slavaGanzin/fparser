...
const getParserConfig = name =>
  getConfig(name).then(config => {
    ...
    return (global.verbosity > 2 ? pp : identity)(deepmerge.all(map(getConfigPath, paths)))
  })
...
const parse = ({name, placeholders}) =>
  Promise.all([parsers, rules])
    .then(([paths, rules]) => rules(name) || reduce((a, x) => a || find(x, paths), null, prepend(test(RegExp(name, 'i')), subdomainIterator(name))) || find(test(/fallback.yaml/), paths))
