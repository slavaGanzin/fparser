...
const getConfig = path =>
  Promise.all(requirePath([path, 'parsers/default.yaml', `${DIR}/default.yaml`]))
    .then(deepmerge.all)

const getParserConfig = name =>
  getConfig(name).then(config => {
    const paths = [`${DIR}/default`, 'parsers/default', name.replace(/\w+.\w+$/, 'default'), name]
...
const parsers = fastGlob(`${DIR}/**/*.yaml`)
